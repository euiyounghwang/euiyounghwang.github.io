...
- __<i>Tools (Pycharm, Eclipse, Postman, Spring Boot, VS Code, Elastic Stacks such as Elasticsearch from v.1.7.3, Logstash, Kibana, and Metricbeat, Apache Kafka, Kafdrop, Apache Tika, Jupyter, Databricks, Datadog, Grafana, Cerebro, Prometheus, AlertManager with Slack alert, Oracle, MongoDB, Postgres, Redis, RabbitMQ, Apache Tomcat, Bonsai for Elasticsearch Cluster, Opensearch, Kibana/Opensearch-dashboard, Jupyter, Google Colab for testing the AI model, Google Search Appliance, Bash Shell, Jira/Confluence, Postman/Insomnia for validating Restful API, K9s for kubernetis, Docker/Docker-Compose (i.e Docker with Search Engine - <i>https://github.com/euiyounghwang/python-search_engine/blob/master/docker-compose.yml</i>), DockerHub with my account)</i>__
